...
import {
  adminGetAllUsers, adminSetUserActive, adminSetUserPaid,
  adminGetWatchlist, adminUpdateWatchlist,
  adminGetStrategy, adminUpdateStrategy
} from '../api';
...
const AdminDashboard = ({ user, t }) => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try { setUsers(await adminGetAllUsers()); } catch (e) { console.error(e); } finally { setUsersLoading(false); }
  }, []);

  const fetchWatchlist = useCallback(async () => {
    setWatchlistLoading(true);
    try {
      const data = await adminGetWatchlist();
  ...
  const fetchStrategy = useCallback(async () => {
    ...
    try { const data = await adminGetStrategy(); setScript(data.script || ''); }
  ...
  const toggleUserActive = async (uid, current) => { await adminSetUserActive(uid, !current); fetchUsers(); };
  const toggleUserPaid = async (uid, current) => { await adminSetUserPaid(uid, !current); fetchUsers(); };

  const saveWatchlist = async () => {
    const symbols = watchlistInput.split('\n').map(s => s.trim().toUpperCase()).filter(Boolean);
    setWatchlistStatus('saving');
    try { await adminUpdateWatchlist(symbols); setWatchlist(symbols); setWatchlistStatus('success'); }
    catch { setWatchlistStatus('error'); }
  };

  const saveStrategy = async () => {
    setScriptStatus('saving');
    try { await adminUpdateStrategy(script); setScriptStatus('success'); }
  ...
  return (
    <div className="dashboard-wrapper">
      <div className="admin-header">
        <h2>{t.admin_title}</h2>
        <p style={{ color: 'var(--text-muted)' }}>{t.admin_logged_as}{user.email}</p>
      </div>
                      ...
                      <div className="user-card-header" onClick={() => setExpandedUser(expandedUser === u.uid ? null : u.uid)}>
                        <div>
                          ...
                          <div className="user-badges">
                            <span className={`badge ${u.isPaid ? 'badge-success' : 'badge-fail'}`}>{u.isPaid ? t.admin_paid : t.admin_unpaid}</span>
                            <span className={`badge ${u.isActive ? 'badge-buy' : ''}`} style={!u.isActive ? { background: '#f5f5f5', color: '#888' } : {}}>{u.isActive ? t.admin_active : t.admin_paused}</span>
                          </div>
                        </div>
                      ...
                      {expandedUser === u.uid && (
                        <div className="user-card-detail">
                          <div className="detail-section">
                            ...
                            {(u.orders || []).length === 0
                              ? <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{t.admin_orders_empty}</p>
                              : (
          ...
          <div>
            <div className="section-header">
              ...
              <button className="btn btn-black" onClick={saveWatchlist} disabled={watchlistLoading || watchlistStatus === 'saving'}>
                {watchlistStatus === 'saving' ? <Loader2 size={16} className="animate-spin" /> : watchlistStatus === 'success' ? <Check size={16} /> : t.admin_save}
              </button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
              <div>
                <textarea className="form-control code-editor" rows={20}
                  value={watchlistInput} onChange={e => { setWatchlistInput(e.target.value); setWatchlistStatus(''); }}
                  placeholder="每行一個代碼&#10;TXF&#10;NVDA&#10;2330"
          ...
          <div>
            <div className="section-header">
              ...
              <button className="btn btn-black" onClick={saveStrategy} disabled={scriptLoading || scriptStatus === 'saving'}>
                {scriptStatus === 'saving' ? <Loader2 size={16} className="animate-spin" /> : scriptStatus === 'success' ? <Check size={16} /> : t.admin_save_script}
              </button>
            </div>
            ...
            <textarea className="form-control code-editor" rows={32}
              value={script} onChange={e => { setScript(e.target.value); setScriptStatus(''); }}
